...
export let options = {
  stages: [
    ...
    { duration: "5m", target: 400 }, // Increase to 300 users
    { duration: "5m", target: 400 }, // Sustain 300 users
    { duration: "5m", target: 550 }, // Increase to 300 users
    { duration: "5m", target: 550 }, // Sustain 300 users
    { duration: "5m", target: 700 }, // Increase to 300 users
    { duration: "5m", target: 700 }, // Sustain 300 users
    { duration: "5m", target: 850 }, // Increase to 300 users
    { duration: "5m", target: 850 }, // Sustain 300 users
    { duration: "5m", target: 500 }, // Increase to 300 users
    { duration: "5m", target: 500 }, // Sustain 300 users
...
export default function () {
  ...
  while (new Date().getTime() - sessionStart < 30 * 60 * 1000) {
    // Run session for ~15 minutes
    // Upload Offline Audio
    let offlineAudio = http.get(
    ...
    check(offlineAudio, {
      "📝 Offline Audio Uploaded": (r) => r.status === 200,
    });

    sleep(Math.random() * 60 + 120);
    // 3.1 Navigate to Speech History (~every 2 - 3 minutes after uploading)
