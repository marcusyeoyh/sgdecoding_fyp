...
export let options = {
  stages: [
    ...
    { duration: "5m", target: 100 }, // Ramp up to 100 users
    { duration: "5m", target: 200 }, // Ramp up to 100 users
...
export default function () {
  ...
  while (new Date().getTime() - sessionStart < 15 * 60 * 1000) {
    ...
    // ===== 4. EDIT NOTE (~every 1-2 minutes) =====
    sleep(Math.random() * 60 + 60);
    let editRes = http.get(
    ...
    check(editRes, { "✏️ Note edited": (r) => r.status === 200 });

    // ===== 5. UPDATE NOTE (~every 3-4 minutes) =====
    sleep(Math.random() * 120 + 180);
    let updatePayload = JSON.stringify({
      _id: "a83c4c5c-a97c-4be1-bf40-f12d670de30b",
      text: JSON.stringify([{ insert: "awdawdawd\n" }]), // Fix here!
    });
    let updateRes = http.post(
      "http://a83add0e498624482aea58e7c6947c08-652731971.ap-southeast-1.elb.amazonaws.com/notes/update",
      updatePayload,
      { headers: authHeaders }
    );
    check(updateRes, { "📄 Note updated": (r) => r.status === 200 });
  }
}
