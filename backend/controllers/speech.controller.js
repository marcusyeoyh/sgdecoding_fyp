...
async function getsTranscriptionJobHistory(req, res, next) {
  await axios
    .get(`${process.env.GATEWAY_URL}/speech/history`, {
      responseType: "json",
      headers: {
        Authorization: `${req.headers.authorization}`,
        "Access-Control-Allow-Origin": "*",
    ...
    .catch((error) => {
      ...
      resPayload = error.response.data || {
...
async function getTranscriptionInJson(req, res, next) {
  ...
  const reqFileStr = new Date().toISOString().replace(/:/g, "_").slice(0, -5); // Remove problematic characters
  ...
  await axios
    ...
    .then((dlZippedFileRes) => {
      ...
      fileWriter.on("close", async () => {
        ...
        yauzl.open(`${filePath}.zip`, { lazyEntries: true }, (err, zipfile) => {
          // if (err) throw err;
          if (err) {
            console.error("[ERROR] File not found, returning 500:", err);
            return res
              .status(200)
              .json({ error: "File missing, please retry." });
          }
          zipfile.readEntry();
          let srtFileName = "";
